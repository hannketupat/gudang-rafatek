...
// jika user sudah login, maka jalankan perintah untuk insert
  ...
  if (isset($_POST['simpan'])) {
    ...
    $query = mysqli_query($mysqli, "SELECT username, email FROM tbl_user WHERE username='$username' OR email='$email'")
    ...
    if ($rows <> 0) {
      ...
      if ($data['username'] == $username) {
        // alihkan ke halaman user dan tampilkan pesan gagal simpan data untuk username
        header("location: ../../main.php?module=user&pesan=4&username=$username");
      } else {
        // alihkan ke halaman user dan tampilkan pesan gagal simpan data untuk email
