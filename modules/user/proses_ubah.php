...
// jika user sudah login, maka jalankan perintah untuk update
  ...
  if (isset($_POST['simpan'])) {
    ...
    // mengecek "username" dan "email" untuk mencegah data duplikat
    // sql statement untuk menampilkan data "username" dan "email" dari tabel "tbl_user" berdasarkan input "username" atau "email"
    $query = mysqli_query($mysqli, "SELECT username, email FROM tbl_user WHERE (username='$username' OR email='$email') AND id_user!='$id_user'")
                                    or die('Ada kesalahan pada query tampil data : ' . mysqli_error($mysqli));
    // ambil jumlah baris data hasil query
    $rows = mysqli_num_rows($query);

    // cek hasil query
    // jika "username" atau "email" sudah ada di tabel "tbl_user"
    if ($rows <> 0) {
      // ambil data yang duplikat
      $data = mysqli_fetch_assoc($query);
      if ($data['username'] == $username) {
        // alihkan ke halaman user dan tampilkan pesan gagal ubah data untuk username
        header("location: ../../main.php?module=user&pesan=4&username=$username");
      } else {
        // alihkan ke halaman user dan tampilkan pesan gagal ubah data untuk email
        header("location: ../../main.php?module=user&pesan=5&email=$email");
      }
    }
    // jika "username" belum ada di tabel "tbl_user"
      ...
      // jika password diubah
        ...
        // sql statement untuk update data di tabel "tbl_user" berdasarkan "id_user"
        $update = mysqli_query($mysqli, "UPDATE tbl_user
                                         SET nama_user='$nama_user', username='$username', email='$email', password='$password_hash', hak_akses='$hak_akses'
                                         WHERE id_user='$id_user'")
                                         or die('Ada kesalahan pada query update : ' . mysqli_error($mysqli));
      }
